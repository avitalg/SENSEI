...
describe('Single source of truth — canonical symbols defined once', () => {
  const CANONICAL: Record<string, string> = {
    ...
    DOCS: 'src/data/catalogs.ts', RES: 'src/data/catalogs.ts', NOTIFS: 'src/data/catalogs.ts',
    // core utils + nav
...
describe('Copy integrity — patient-merge flow makes no unbacked archive/audit claim', () => {
  // confirmMerge() removes the duplicate from state — there is no archivedPatients
  // store and the Privacy "Audit Log" is a static fixture nothing writes to. So the
  // merge copy must not tell users the record is "archived / restorable" or "logged
  // to the activity log" (a truthfulness/traceability regression fixed in 1.0.13).
  // Scoped to merge-flow lines only, so the legitimate notification archive and the
  // separate (documented) delete flows are unaffected.
  it('merge dialog + confirmMerge toast claim neither archival nor activity-log logging', () => {
    const src = readFileSync(join(SRC, 'components/layout/Dialogs.tsx'), 'utf8')
    const MERGE_ANCHOR = /merge(Archive|Canon|Total)|הרשומות מוזגו|לאחר המיזוג|שאר הרשומות/
    const FORBIDDEN = /לארכיון|ביומן הפעילות|תתועד|יתועד/
    const offenders = src.split('\n')
      .filter((l) => MERGE_ANCHOR.test(l) && FORBIDDEN.test(l))
      .map((l) => l.trim().slice(0, 90))
    expect(offenders, 'merge copy must match confirmMerge behavior (record removed — not archived or logged)').toEqual([])
  })
...
describe('Contrast — on-accent surfaces adapt per theme', () => {
  ...
  it('a print stylesheet isolates the clinical letter (window.print must not print app chrome)', () => {
    ...
    for (const sel of ['.app-sidebar', '.appbar', '.shell-fab', '.no-print']) {
