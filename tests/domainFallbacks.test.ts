...
describe('session-count derivation', () => {
  ...
  it('an explicit count is honored; a non-positive one falls through to the deterministic hash', () => {
    expect(demoSessionCount({ id: OUTSIDE_ID, sessions: 3 })).toBe(3);
    const zero = demoSessionCount({ id: OUTSIDE_ID, sessions: 0 });
    expect(zero).toBeGreaterThanOrEqual(6); // hash range 6–10, capped by SESSION_DATES
    expect(zero).toBeLessThanOrEqual(10);
    // deterministic: the same id always yields the same count
    expect(demoSessionCount({ id: OUTSIDE_ID })).toBe(demoSessionCount({ id: OUTSIDE_ID }));
  });
});
...
describe('buildPatientSessions', () => {
  ...
  it('a risk-free session shows no chip, while a flagged one does', () => {
    const outside = buildPatientSessions({ id: OUTSIDE_ID, name: 'x' }, [], ctx);
  ...
  it('durations come from the dataset for repository patients, and are derived otherwise', () => {
    ...
    expect(buildPatientSessions({ id: OUTSIDE_ID, name: 'x' }, [], ctx)[0].duration).toMatch(/^\d+ דק׳$/);
  });
...
describe('session-detail content — repository values vs the neutral fallback', () => {
  ...
  it('insight and title: bespoke for repository patients, generic/empty otherwise', () => {
    expect(sessionInsight({ id: REPO_ID }, 0)).toBeTruthy();
    expect(sessionInsight({ id: OUTSIDE_ID }, 0)).toBeTruthy(); // generic pool, never empty
