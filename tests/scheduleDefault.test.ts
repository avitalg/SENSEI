...
describe('defaultScheduleForm — working-hours default', () => {
  it('daytime: next 30-min slot', () => {
    expect(defaultScheduleForm('aladdin', at(10, 5)).time).toBe('10:30');
    expect(defaultScheduleForm('aladdin', at(14, 40)).time).toBe('15:00');
  });

  it('small hours (before 08:00) → 09:00 same day', () => {
    for (const h of [0, 2, 5, 7]) {
      const f = defaultScheduleForm('aladdin', at(h, 15));
  ...
  it('evening (>= 20:00) → 09:00 the next day', () => {
    const now = at(21, 10);
    const f = defaultScheduleForm('aladdin', now);
    expect(f.time).toBe('09:00');
  ...
  it('the default time is never outside 08:00–20:00', () => {
    for (let h = 0; h < 24; h++) {
      const h2 = hour(defaultScheduleForm('aladdin', at(h, 25)).time);
