...
// Freeze "today" to a fixed date so the demo's date-pinned appointment (Simba, on
// 2026-07-21) never lands on today and shifts the agenda. Fake only Date, so the
// setTimeout-based settle() still runs.
beforeEach(() => {
...
// A function so it reads the frozen clock set in beforeEach, not the import-time date.
const makeTodayAppt = () => ({ id: 'tts1', pid: 'p1', date: todayKey(), time: '23:59', dur: 50, description: '', status: 'upcoming' });
...
describe('home agenda — per-session recap TTS (spec 1.2)', () => {
  it('speaks the patient name and previous-session summary, and stops on second press', async () => {
    ...
    mount({ view: 'app', route: 'dashboard', onboardTipDismissed: true, scheduledAppts: [makeTodayAppt()] });
    await settle();
