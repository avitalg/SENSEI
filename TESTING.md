...
```bash
npm test               # full suite (vitest run) — 25 files, 215 tests
...
| Suite | Covers |
|---|---|
| `utils`, `searchUtils`, `dedup`, `pager` | pure logic: risk/avatar/file-validation/recent-patient helpers, search ranking, duplicate clustering + canonical selection, pagination |
...
with a 70% threshold across statements/branches/functions/lines (currently ~87% / ~79%).
