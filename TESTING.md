...
| `domainFallbacks` | domain-layer fallback/edge paths a repository patient never exercises: therapist-created patients, blank email/address, malformed dates, archive/restore purity, deterministic session counts, session deletion, and the neutral content every non-repository id falls back to |
