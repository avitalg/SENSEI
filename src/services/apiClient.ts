...
//
// STATUS: dormant. This app currently runs on seed data + localStorage (see
// ARCHITECTURE.md). This client is inert unless `VITE_API_BASE_URL` is set, so
// wiring a backend later is a config change — not a rewrite. Nothing calls it
// today; existing flows are unchanged.
//
// Design: native fetch (no axios dep), Bearer auth via a pluggable token
// provider (so tokens are never hardcoded or stored here), per-request timeout
// + AbortController, and consistent typed errors.
