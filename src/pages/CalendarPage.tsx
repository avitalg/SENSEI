// Calendar — ported from 'Sensei demo.dc.html'
// (template lines 1568–1684 · integration logic ~2875–3010 · view model in
//  renderVals ~4316–4539). The Google Calendar feed is a frontend-only
//  simulation: OAuth/token-refresh/permissions are server-side in production,
//  and the backend route returns live events. In this sandbox the fetch falls
//  back to the integration fixture below.
//
// Per the assignment, all cal* view-state is kept LOCAL to this page via
// useState — it was ephemeral view-state in the prototype (not in PERSIST_KEYS),
...
const weekStart = (d: Date) => { const s = new Date(d); s.setHours(0, 0, 0, 0); s.setDate(s.getDate() - s.getDay()); return s }
const weekEnd = (d: Date) => { const e = weekStart(d); e.setDate(e.getDate() + 7); return e }
...
// ---- Google event normalization (ported verbatim) ----
function normalizeGoogleEvents(items: any[]) {
  return (items || [])
    ...
    .map((e) => {
      ...
      return {
        id: e.id, title: e.summary || '(ללא כותרת)', description: e.description || '', location: e.location || '',
        htmlLink: e.htmlLink || '', meetLink: meet, allDay, start, end, status: e.status || 'confirmed',
        attendees: (e.attendees || []).map((a: any) => ({ name: a.displayName || a.email || '', email: a.email || '', self: !!a.self, response: a.responseStatus || 'needsAction' })),
      }
    })
    .sort((a, b) => +a.start - +b.start)
}

// ---- integration fixture (sandbox fallback; live data replaces this in prod) ----
async function calFixture() {
  await new Promise((r) => setTimeout(r, 650))
  const base = weekStart(new Date())
  const iso = (off: number, h: number, m: number) => { const d = new Date(base); d.setDate(base.getDate() + off); d.setHours(h, m, 0, 0); return d.toISOString() }
  const dayStr = (off: number) => { const d = new Date(base); d.setDate(base.getDate() + off); return dayKey(d) }
  const ev = (id: string, off: number, sh: number, sm: number, eh: number, em: number, summary: string, extra?: any) =>
    Object.assign({ id, status: 'confirmed', summary, start: { dateTime: iso(off, sh, sm) }, end: { dateTime: iso(off, eh, em) }, htmlLink: 'https://calendar.google.com/calendar/r/eventedit/' + id }, extra || {})
...
const CAL_ENDPOINT = '/api/integrations/google-calendar/events'

// Fetch with a 1.2s timeout; genuine cancels re-throw (navigation / superseded refresh).
function fetchJson(url: string, signal?: AbortSignal): Promise<any> {
  return new Promise((resolve, reject) => {
    const to = setTimeout(() => reject(new Error('timeout')), 1200)
    if (typeof fetch === 'undefined') { clearTimeout(to); reject(new Error('no fetch')); return }
    fetch(url, { signal, headers: { Accept: 'application/json' }, credentials: 'same-origin' })
      .then((res) => { if (!res.ok) throw new Error('HTTP ' + res.status); return res.json() })
      .then((d) => { clearTimeout(to); resolve(d) })
...
export default function CalendarPage() {
  ...
  const loadCalendar = async (isRefresh: boolean) => {
    ...
    try {
      const now = new Date()
      const events = await apiFetchCalendarEvents({ timeMin: weekStart(now), timeMax: weekEnd(now), signal: ac ? ac.signal : undefined })
  ...
  // ---- start on mount · poll only while mounted AND tab visible · stop on unmount ----
  useEffect(() => {
  ...
  const openScheduleDialog = () =>
    set({ dialog: 'schedule', apptForm: { pid: 'p1', time: '11:00', dur: '50', type: 'פגישה שבועית' }, errors: {} })
  ...
  const calAgenda = dayEvents.map((e) => {
    ...
    return {
      ...
      hasMeet: !!e.meetLink, hasLoc: !!e.location, location: e.location, opacity: ended ? '.72' : '1',
      onOpen: () => { if (e.htmlLink && typeof window !== 'undefined') window.open(e.htmlLink, '_blank', 'noopener,noreferrer') },
      onJoin: (ev?: any) => { if (ev) ev.stopPropagation(); if (e.meetLink && typeof window !== 'undefined') window.open(e.meetLink, '_blank', 'noopener,noreferrer') },
    }
  })
  ...
  const calKpis = [
    ...
    { label: 'שיחות וידאו', value: String(calEvents.filter((e) => e.meetLink).length), color: 'var(--success)' },
    ...
    <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 9, flexWrap: 'wrap' }}>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13, fontWeight: 600, color: 'var(--text-secondary)' }}>
              <span style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--success)', flexShrink: 0 }}></span>מחובר ל־Google Calendar
            </span>
      ...
      {calLoading && (
        <>
          ...
          <div className="rx-kpi4" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14, marginBottom: 20 }}>
            {[1, 2, 3, 4].map((k) => (
      ...
      {calEmptyState && (
        <div style={{ background: 'var(--paper)', border: '1px solid var(--divider)', borderRadius: 12, padding: '56px 24px', textAlign: 'center', boxShadow: CARD_SHADOW }}>
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
            <button className="cal-empty-refresh" onClick={refreshCalendar} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, height: 44, padding: '0 20px', border: '1px solid var(--border-input)', borderRadius: 10, background: 'var(--paper)', fontSize: 14, fontWeight: 600, cursor: 'pointer', color: 'var(--text-2)' }}>
              <svg viewBox="0 0 24 24" width="18" height="18" fill="var(--primary)"><path d="M17.65 6.35A7.958 7.958 0 0 0 12 4c-4.42 0-7.99 3.58-7.99 8s3.57 8 7.99 8c3.73 0 6.84-2.55 7.73-6h-2.08A5.99 5.99 0 0 1 12 18c-3.31 0-6-2.69-6-6s2.69-6 6-6c1.66 0 3.14.69 4.22 1.78L13 11h7V4l-2.35 2.35z" /></svg>רענון
            </button>
            <button className="cal-empty-new" onClick={openScheduleDialog} style={{ display: 'inline-flex', alignItems: 'center', gap: 7, height: 44, padding: '0 20px', border: 'none', borderRadius: 10, background: 'var(--primary)', color: 'var(--paper)', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>
              <svg viewBox="0 0 24 24" width="18" height="18" fill="var(--paper)"><path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z" /></svg>פגישה חדשה
            </button>
          </div>
        </div>
      )}
              ...
              <div key={a.id} className="cal-agenda-row" onClick={a.onOpen} role="button" tabIndex={0} aria-label={a.title} style={{ display: 'flex', alignItems: 'center', gap: 15, padding: '15px 22px', borderBottom: '1px solid var(--line)', cursor: 'pointer', borderInlineStart: '3px solid ' + a.lineColor, opacity: Number(a.opacity) }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginTop: 3 }}>
                    ...
                    {a.hasMeet && (
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11.5, fontWeight: 700, color: 'var(--primary)' }}>
                        <svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor"><path d="M17 10.5V7c0-.55-.45-1-1-1H4c-.55 0-1 .45-1 1v10c0 .55.45 1 1 1h12c.55 0 1-.45 1-1v-3.5l4 4v-11l-4 4z" /></svg>וידאו
                      </span>
                    )}
                    {a.hasLoc && (
                ...
                {a.hasMeet && (
                  <button className="cal-join-btn" onClick={a.onJoin} aria-label="הצטרפות לשיחת וידאו" style={{ height: 36, padding: '0 14px', border: '1px solid var(--primary-border)', borderRadius: 10, background: 'var(--primary-surface)', color: 'var(--primary)', cursor: 'pointer', fontSize: 12.5, fontWeight: 700, fontFamily: 'inherit', whiteSpace: 'nowrap', flexShrink: 0 }}>הצטרפות</button>
                )}
              </div>
            ))}
          </div>
