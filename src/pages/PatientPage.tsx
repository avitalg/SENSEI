      ...
      {!S.loading && (
        <div>
          ...
          <section role="tabpanel" aria-label="סקירת מטופל" hidden={tab !== 'overview'} className="pw-panel">
              <div className="pw-readable pd-overview" style={{ background: 'var(--paper)', border: '1px solid var(--divider)', borderRadius: 12, boxShadow: CARD_SHADOW }}>
                <div className="pd-overview-head">
                  <div>
                    <h2 style={{ margin: 0, fontSize: 17, fontWeight: 800 }}>סקירת מטופל</h2>
                    <p className="pd-overview-subtitle">תמונה קלינית מרוכזת לקראת המשך הטיפול</p>
                  </div>
                  {S.editingOverview ? (
                    <div className="pd-overview-actions">
                      <button onClick={saveOverview} className="pd-overview-save">שמירה</button>
                      <button onClick={cancelOverview} className="pd-overview-cancel">ביטול</button>
                    </div>
                  ) : (
                ...
                <dl className="pd-overview-grid">
                  {OVERVIEW_FIELDS.map((f) => (
                    <div key={f.key} className={`pd-overview-card pd-overview-card--${f.key}`}>
                      <dt className="pd-overview-label"><span aria-hidden="true" className="pd-overview-dot" />{f.label}</dt>
                      {S.editingOverview ? (
                        <dd className="pd-overview-value"><textarea value={overviewDraft[f.key]} onChange={onOverviewField(f.key)} aria-label={f.label} className="pd-overview-field" /></dd>
                      ) : (
                        <dd className="pd-overview-value">{overview[f.key] || 'לא צוין'}</dd>
                      )}
                    </div>
                  ))}
                </dl>
