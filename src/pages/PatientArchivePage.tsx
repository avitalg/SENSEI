...
export default function PatientArchivePage() {
  ...
  const rows = filtered.map((p: any) => {
    ...
    const email = displayPatientEmail(p.email);
    return {
      ...p,
      avBg: a.bg,
      avColor: a.color,
  ...
  return (
    <div className="pa-page" style={{ maxWidth: 1200, margin: '0 auto' }}>
      <div className="pa-header" style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 22, gap: 16, flexWrap: 'wrap' }}>
        ...
        <button
          ...
          className="pat-new-btn pa-back-btn"
      ...
      {!loading && archived.length > 0 && (
        <div className="pa-search" style={{ position: 'relative', marginBottom: 16 }}>
      ...
      <div className="pa-list" style={{ background: 'var(--paper)', border: '1px solid var(--divider)', borderRadius: 10, boxShadow: CARD_SHADOW, overflow: 'hidden' }}>
        {loading && (
        ...
        {!loading && archived.length === 0 && (
          <div className="pa-empty" style={{ padding: '52px 24px', textAlign: 'center' }}>
        ...
        {!loading && rows.map((p: any) => (
          ...
          >
            ...
            >
              <span className="pa-av" style={{ width: 44, height: 44, borderRadius: '50%', background: p.avBg, color: p.avColor, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 15, flexShrink: 0, opacity: 0.85 }}>{p.initials}</span>
              <div className="pa-row-body" style={{ flex: 1, minWidth: 0 }}>
                <div className="pa-row-name" style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', marginBottom: 3 }}>
                  <span style={{ fontSize: 15, fontWeight: 700, color: 'var(--text)' }}><Highlight text={p.name} query={query} /></span>
                  <span className="pa-badge">ארכיון</span>
                </div>
                <div className="pa-row-meta" style={{ fontSize: 13, color: 'var(--text-secondary)' }}>
                  <span className="pa-meta-contact" dir="ltr">
                    {[p.phone, p.email].filter(Boolean).join(' · ') || '—'}
                  </span>
                  <span className="pa-meta-sep"> · </span>
                  <span className="pa-meta-since">טיפול: {p.since}</span>
                </div>
              </div>
            </button>
            <div className="pa-row-actions" style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
              <button
                type="button"
                onClick={p.onRestore}
                disabled={restoringId === p.id}
                aria-label="שחזור מטופל"
                className="pat-icon-btn tap44 pa-restore-btn"
                style={{ height: 34, padding: '0 12px', border: '1px solid var(--primary-border)', borderRadius: 8, background: 'var(--primary-surface)', cursor: restoringId === p.id ? 'wait' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--primary)', fontSize: 12.5, fontWeight: 700, opacity: restoringId === p.id ? 0.7 : 1 }}
              >
                {restoringId === p.id ? 'משחזר…' : 'שחזור'}
              </button>
              <button
                type="button"
                onClick={(e) => { e.stopPropagation(); set({ dialog: 'deletePatientPermanent', dialogPatientId: p.id }); }}
                aria-label="מחיקת מטופל לצמיתות"
                className="pat-del-btn"
                style={{ width: 34, height: 34, border: '1px solid var(--divider)', borderRadius: 8, background: 'var(--paper)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
              >
