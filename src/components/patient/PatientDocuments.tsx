      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        {filtered.map((doc) => (
          <div key={doc.id} style={{ display: 'flex', alignItems: 'center', gap: 10, border: '1px solid var(--line)', borderRadius: 9, padding: '9px 11px' }}>
            ...
            <select value={doc.category} onChange={(e) => setCategory(doc.id, e.target.value)} aria-label={'קטגוריה · ' + doc.name} style={{ height: 28, border: '1px solid var(--border-input)', borderRadius: 7, background: 'var(--primary-surface)', color: 'var(--text-2)', fontSize: 12.5, fontWeight: 600, cursor: 'pointer', flexShrink: 0, maxWidth: 108 }}>
