...
export function buildCmdRoutes(app: { set: (p: any) => void; navigate: (r: string, p?: any) => void; firstPatientId?: string }): CmdRoute[] {
  ...
  const CMD_ACTIONS: CmdRoute[] = [
    ...
    { label: 'העלאת הקלטה חדשה', icon: 'M9 16h6v-6h4l-7-7-7 7h4v6zm-4 2h14v2H5v-2z', go: () => navigate('upload', { uploadPatientFixed: false, upload: { state: 'idle', progress: 0, fileName: '', error: '' } }) },
