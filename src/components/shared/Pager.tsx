        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
          ...
          <div role="group" aria-label="שורות בעמוד" style={{ display: 'inline-flex', border: '1px solid var(--divider)', borderRadius: 8, overflow: 'hidden' }}>
            {p.sizeOpts.map((s: any) => (
              <button key={s.n} onClick={s.onClick} aria-pressed={s.active} className="pager-size-btn tap44" style={{ height: 30, minWidth: 34, padding: '0 9px', border: 'none', borderInlineStart: '1px solid var(--divider)', background: s.bg, color: s.color, fontSize: 12.5, fontWeight: s.weight, cursor: 'pointer', fontFamily: 'inherit' }}>{s.n}</button>
            ))}
          </div>
        </div>
      </div>
      <div style={{ display: 'flex', gap: 5, alignItems: 'center' }}>
        <button onClick={p.onPrev} disabled={p.prevDisabled} aria-label="עמוד קודם" className="pager-nav-btn tap44" style={{ width: 34, height: 34, border: '1px solid var(--divider)', borderRadius: 8, background: 'var(--paper)', cursor: p.prevCursor, color: 'var(--text-muted)', opacity: p.prevOpacity, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        {p.pageItems.map((pg: any, i: number) => pg.gap
          ...
          : <button key={pg.n} onClick={pg.onClick} aria-current={pg.ariaCurrent} aria-label={'עמוד ' + pg.n} className="pager-page-btn tap44" style={{ minWidth: 34, height: 34, padding: '0 9px', border: '1px solid ' + pg.border, borderRadius: 8, background: pg.bg, color: pg.color, cursor: 'pointer', fontWeight: pg.weight, fontSize: 13.5, fontFamily: 'inherit' }}>{pg.n}</button>)}
        <button onClick={p.onNext} disabled={p.nextDisabled} aria-label="עמוד הבא" className="pager-nav-btn tap44" style={{ width: 34, height: 34, border: '1px solid var(--divider)', borderRadius: 8, background: 'var(--paper)', cursor: p.nextCursor, color: 'var(--text-muted)', opacity: p.nextOpacity, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
