...
// Relevance score for a patient against a query. `tagsMap` is the patientTags
// record (id → string[]). Higher = better; 0 = no match.
export function scoreP(p: any, q: string, tagsMap: Record<string, string[]> = {}): number {
  const qq = normHe(q); if (!qq) return 0
  const name = normHe(p.name), focus = normHe(p.focus), tags = normHe(((tagsMap || {})[p.id] || []).join(' '))
  const phone = (p.phone || '').replace(/\D/g, ''), email = (p.email || '').toLowerCase()
  if (name === qq) return 7
  if (name.startsWith(qq)) return 6
  if (name.includes(qq)) return 5
  if (tags.startsWith(qq)) return 4.5
  if (focus.startsWith(qq)) return 4
  if (focus.includes(qq)) return 3
  if (tags.includes(qq)) return 2.5
