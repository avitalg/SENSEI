...
### Changed
- **התקציר הקולי בדוח ההכנה מקריא באמת את הדוח.** עד כה הכרטיס הריץ אנימציית
  נגן עד "1:48" בלי שהושמע דבר. כעת הוא מקריא בקול את תוכן הדוח — סקירה מהירה,
  סיכום הפגישה הקודמת, נקודות למעקב, מטרות ושאלות מוצעות — באותה הקראה
  מובנית בדפדפן שמשמשת את הסיכום היומי ואת תקצירי המטופל.
- מד ההתקדמות מתעדכן לפי מיקום ההקראה בפועל. בדפדפן שאינו מדווח על מיקום,
  הפסים פועמים והשעון מוסתר במקום להציג זמן מדומה. משך ההקראה מוצג כהערכה
  ("כ-2:10 דקות").
- הכרטיס מציג "הקראה קולית אינה נתמכת בדפדפן זה" כשאין תמיכה, במקום כפתור מת.
- ההקראה נעצרת במעבר למטופל אחר וברענון הדוח, כדי שלא תישמע על תוכן שהתחלף.

## [1.62.6] — 2026-07-25

### Added
- **גרף התקציר הקולי חזר לדוח ההכנה, והפעם הוא נע לפי ההקראה עצמה.** הגרף הוסר
  כשהתקציר המדומה הוחלף בהקראה אמיתית, כי הוא תיאר אודיו שלא היה קיים. הוא חוזר
  כרכיב משותף (`SpeechWaveform`) שממלא את העמודות לפי `progress` חדש ב-
  `useMeetingReportSpeech`: אומדן זמן הקריאה נגזר מאורך הטקסט, נצבע בלולאת
  `requestAnimationFrame`, ונעצר על 99% עד שההקראה מסתיימת באמת — כך שהעמודות
  אינן מכריזות "סיום" באמצע משפט. עצירה או סיום ההקראה מרוקנים את הגרף.
  בדסקטופ הגרף מוצג בכרטיס גם במנוחה (עמודות עמומות); בנייד מוצג גרף מוקטן
  לצד הכפתור רק בזמן ההקראה. שעון ההקראה הקבוע (`1:48`) לא חוזר — משך ההקראה
  הוא אומדן ולא נתון.

## [1.62.5] — 2026-07-25

### Changed
- **"תקציר קולי מהיר" בדוח ההכנה מוקרא כעת בדפדפן, לא מדגמה מדומה.** הכפתור
  בדוח ההכנה בדסקטופ הפעיל בעבר סרגל התקדמות מדומה (`briefPlaying`/
  `briefProgress`) שלא באמת הקריא דבר. הכפתור מריץ כעת `tts.toggle(...)` על
  תמצית הדוח הנוכחית — אותו `useTts` שמפעיל את "סיכום פתיחת היום" וסיכום
  המטופל — דרך הוק משותף חדש, `useMeetingReportSpeech`, כדי שההתנהגות תישאר
  זהה בין הדסקטופ למובייל. מצב הכפתור (הקראה/עצירה) עוקב אחרי `tts.speaking`,
  והכפתור מוסתר כשה-Web Speech API אינו זמין. דוח ההכנה בנייד מקבל את אותו
  כפתור השמעה, שלא היה קיים בו קודם.
...
- **התאמת גרסאות.** `package.json` נותר על 1.62.1 בעוד ה-CHANGELOG כבר עודכן
  ל-1.62.2, ולכן שומר עקביות הגרסאות נכשל. שלושת המקורות מסונכרנים שוב.
