...
- **משימות להמשך גם בתיק המטופל.** לשונית "סקירה" בתיק (במחשב ובמובייל, רכיב משותף `PatientFollowups`) מציגה את המשימות הפתוחות של אותו מטופל בלבד — כותרת, תיאור מילה-במילה, תג עדיפות ותאריך פגישת המקור — וכל משימה נפתחת ישירות בפגישה שממנה נוצרה.

- **תיעוד המטפל בעמוד הפגישה.** עמוד פרטי הפגישה מציג כעת גם את ההקלטה המדוברת של המטפל, את ההערה הקלינית המצורפת ואת מוקד "להמשך" — מילה-במילה מקובץ `recorded_sessions.md` של המאגר, משויכים לפגישה הנכונה בלבד (אינדקס מדויק, ללא מחזוריות). למטופלים שאינם מהמאגר לא מוצג דבר — שום תוכן אינו מומצא.

- **חיפוש מעמיק ומדויק יותר.** תוצאות פגישות בחיפוש הכולל מאתרות כעת גם תובנות, נושאים מרכזיים ואת תיעוד המטפל (הקלטה · הערה קלינית · "להמשך") — כל מה שעמוד הפגישה מציג — ולחיצה על תוצאה פותחת את הפגישה שנמצאה עצמה (במקום עמוד הסיכום הכללי של המטופל). נוספה קטגוריית "משימות": משימות המשך פתוחות מהמאגר נמצאות לפי כותרת/תיאור ונפתחות בפגישת המקור.

- **משימות להמשך גם בדף הבית בטלפון.** מסך היום במובייל מציג רצועת משימות פתוחות (עד שלוש, מהמאגר, למטופלים פעילים בלבד) עם ניווט ישיר לפגישת המקור — יישור קו עם כרטיס דף הבית במחשב. נגזרת `openRepoTasks` מחושבת כעת פעם אחת (התוכן קבוע בזמן בנייה) ומשותפת לכל המסכים.

### Fixed
- **קישור "עריכת הסיכום" מוצג רק בפגישה האחרונה.** עורך הסיכום המלא הוא ברמת המטופל ועורך תמיד את סיכום הפגישה האחרונה; בפגישות מוקדמות הקישור היה פותח סיכום של פגישה אחרת (יעד שגוי) — כעת הוא מוצע רק כשהפגישה המוצגת היא אכן האחרונה.

### Tests
- חבילה חדשה `repoDatasetClean` — שומר תקינות למאגר: אפס תקלות פירוש בכל המאגר המוטמע, שלמות שדות לכל מטופל ופגישה (שם · גישה · רקע · תאריך · שעה · סיכום · הקלטה · תובנה · נושאים), רציפות מספרי פגישות 1..N, ייחודיות מזהי משימות והפניות לפגישות קיימות. סנכרון עתידי פגום של המאגר ייכשל ב-CI במקום להתדרדר בשקט.
- `sessionTherapistDoc` הורחבה — הקישור לעריכת הסיכום מופיע רק בפגישה האחרונה ונעדר מפגישות מוקדמות.
- `followupTasks` הורחבה — רצועת המשימות בדף הבית במובייל (מסך היום) עם ניווט לפגישת המקור.
- `searchSessions` הורחבה — התאמת תוכן מהקלטת המטפל (מחרוזת שקיימת רק ב-recorded_sessions.md) עם ניווט לפגישה שנמצאה, וקטגוריית משימות עם ניווט לפגישת המקור.
- חבילה חדשה `sessionTherapistDoc` — מיפוי מדויק של הקלטה/הערה/מוקד לכל פגישה בכל תיקיות המאגר, איפוס מחוץ לטווח ולמטופלים שאינם במאגר, ורינדור המקטע בעמוד הפגישה עם התוכן המילולי.
- חבילה חדשה `followupTasks` — נגזרת `openRepoTasks` (פגישה אחרונה בלבד, ללא כפילויות, תת-קבוצה של `repoTasks`, תיאור מילה-במילה מהמאגר, ללא תאריכי יעד), רינדור הכרטיס בדף הבית, קישור עומק לפגישת המקור, הסתרת הכרטיס כשאין מטופלי מאגר פעילים, והצגת משימות המטופל בלשונית הסקירה בתיק כולל ניווט לפגישת המקור.
